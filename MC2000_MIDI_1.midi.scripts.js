...
mc2000.init = function (id, debug) {
    ...
    for (i = 1; i <= 2; i++) {
        ...
        engine.connectControl(
            "[Channel" + i + "]",
            "keylock",
            "mc2000.keylockSetLed"
        );
        // Sync
        engine.connectControl(
            "[Channel" + i + "]",
            "beat_active",
            "mc2000.beatActiveSetLed"
        );
        ...
        engine.connectControl(
            "[Channel" + i + "]",
            "loop_start_position",
            "mc2000.loopStartSetLed"
        );
        // Loop out
        engine.connectControl(
            "[Channel" + i + "]",
            "loop_end_position",
            "mc2000.loopEndSetLed"
        );
        // Auto loop (actually reloop/exit)
        engine.connectControl(
            "[Channel" + i + "]",
            "loop_enabled",
            "mc2000.loopEnableSetLed"
        );
...
mc2000.beatJump = function (group, beats, forward) {
    ...
    var backsamples =
        backseconds * engine.getValue(group, "track_samplerate") * 2;
    ...
    engine.setValue(
        group,
        "playposition",
        newpos / engine.getValue(group, "track_samples")
    );
};
...
// === HOT CUES ===
mc2000.hotcueActivateOrDelete = function (
    channel,
    control,
    value,
    status,
...
mc2000.hotcueSetLed = function (value, group, control) {
    ...
    mc2000.setLed(
        mc2000.group2Deck(group),
        mc2000.leds["cue" + control[7]],
        value
    );
};
